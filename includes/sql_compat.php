...
function finance_compat_sql(string $sql): string
{
    ...
    // ══ INSERT INTO shares ══════════════════════════════════════════════════
    // PHP sends:   INSERT INTO shares (date, member, amount) VALUES (...)
    // Target:      INSERT INTO shares (share_date, member_id, amount) VALUES (...)
    if (preg_match('/\bINSERT\s+INTO\s+shares\b/i', $sql)) {
        ...
        // Subselect phone → id. NOTE: FROM routing already changed "members" →
        // "app_members", so we match both names. app_members now exposes id.
        $sql = preg_replace(
            '/\(\s*SELECT\s+mobilenumber\s+FROM\s+(?:app_members|members)\s+WHERE\s+mobilenumber\s*=\s*\'([^\']*)\'\s*\)/i',
            "(SELECT id FROM app_members WHERE mobilenumber='$1')",
            $sql
        );
    }

    // ══ INSERT INTO loans ═══════════════════════════════════════════════════
    // PHP sends:   INSERT INTO loans (date, member, amount, interest) VALUES (...)
    // Target:      INSERT INTO loans (loan_date, member_id, amount, interest_rate) VALUES (...)
    if (preg_match('/\bINSERT\s+INTO\s+loans\b/i', $sql)) {
        ...
        // Subselect phone → id (same note as shares above)
        $sql = preg_replace(
            '/\(\s*SELECT\s+mobilenumber\s+FROM\s+(?:app_members|members)\s+WHERE\s+mobilenumber\s*=\s*\'([^\']*)\'\s*\)/i',
            "(SELECT id FROM app_members WHERE mobilenumber='$1')",
            $sql
    ...
    // PHP sends:   INSERT INTO loanPayments (paymentID, date, member, amount)
    // Target:      INSERT INTO loanpayments (paymentid, payment_date, member_phone, amount)
    ...
    if (preg_match('/\bwhere\s+member\s*=/i', $sql)
        ...
        $sql = preg_replace(
            ...
            "WHERE member_id = (SELECT id FROM app_members WHERE mobilenumber='$1')",
            $sql
        );
    }
