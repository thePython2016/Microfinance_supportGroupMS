...
// ── Loan total (sum of total_payable) — same approach as checkLoans.php ──────
$Total = '0.00';
try {
    $loanTotalResult = $connection->pdo->query(
        'SELECT COALESCE(SUM(total_payable), 0) AS grand_total FROM "loans"'
    );
    if ($loanTotalResult) {
        $row   = $loanTotalResult->fetch(PDO::FETCH_ASSOC);
        $Total = number_format((float)($row['grand_total'] ?? 0), 2);
    }
} catch (Exception $e) {
    $Total = '0.00';
}
...
$total = '0.00';
try {
    $shareTotalResult = $connection->pdo->query(
        'SELECT COALESCE(SUM(amount), 0) AS grand_total FROM "shares"'
    );
    if ($shareTotalResult) {
        $row   = $shareTotalResult->fetch(PDO::FETCH_ASSOC);
        $total = number_format((float)($row['grand_total'] ?? 0), 2);
    }
} catch (Exception $e) {
    $total = '0.00';
...
$members = 0;
try {
    $membersCountResult = $connection->pdo->query(
        'SELECT COUNT(*) AS total FROM "members"'
    );
    if ($membersCountResult) {
        $row     = $membersCountResult->fetch(PDO::FETCH_ASSOC);
        $members = (int)($row['total'] ?? 0);
    }
} catch (Exception $e) {
    $members = 0;
}
...
function loadFallbackChartData($connection, array &$shareMonths, array &$shareAmount, array &$months, array &$amount): void
{
    ...
    $shareQuery  = "SELECT TO_CHAR(\"share_date\", 'YYYY-MM') AS month_key, SUM(amount) AS total
                    FROM \"shares\"
    ...
    // Use total_payable for the monthly loan trend chart as well
    $loanQuery  = "SELECT TO_CHAR(\"loan_date\", 'YYYY-MM') AS month_key, SUM(total_payable) AS total
                   FROM \"loans\"
  ...
  <body>
                      ...
                      <div class="row g-6">
                          ...
                          <div class="d-flex align-items-center icon-blockMember">
                            <div class="icon-section">
                              <div class="avatar-initial">
                                <svg xmlns="http://www.w3.org/2000/svg" class="members-icon" viewBox="0 0 640 512" style="width:24px;height:24px;fill:currentColor;">
                        ...
                        <div class="col-md-4 col-12">
                          <div class="d-flex align-items-center icon-blockShares">
                            <div class="icon-section">
                              <div class="avatar-initial">
                                <svg xmlns="http://www.w3.org/2000/svg" class="share-icon" viewBox="0 0 512 512" style="width:24px;height:24px;fill:currentColor;">
                        ...
                        <!-- Loans total (sum of total_payable) -->
                        <div class="col-md-4 col-12">
                          <div class="d-flex align-items-center icon-blockLoan">
                            <div class="icon-section">
                              <div class="avatar-initial">
                                <svg xmlns="http://www.w3.org/2000/svg" class="loan-icon" viewBox="0 0 576 512" style="width:24px;height:24px;fill:currentColor;">
                            ...
                            <div class="ms-3 icon-text">
                              <p class="mb-0">Loans / Total Payable (Tsh)</p>
                              <h5 class="mb-0"><?php echo htmlspecialchars($Total); ?></h5>
                            </div>
                          </div>
                        </div>
                ...
                <div class="col-xl-5 col-md-6">
                  <div class="card h-100">
                    <div class="card-header d-flex justify-content-between align-items-center">
                      <h5 class="card-title m-0" style="color:rgb(61,60,60)!important;font-weight:600;">Loan Status Breakdown</h5>
                    </div>
                ...
                <div class="col-xl-7 col-md-6">
                  <div class="row gy-6">
                    ...
                    <div class="col-12">
                      <div class="card">
                        <div class="card-header">
                          <h5 class="card-title m-0" style="color:rgb(61,60,60)!important;font-weight:600;">Monthly Loan Issuance Trend – Total Payable (TZS)</h5>
                        </div>
                        <div class="card-body">
                          <div class="chart-container" style="height:160px;">
                        ...
                        <tbody>
                          ...
                          foreach ($selectMembers ?: [] as $m) {
                              echo "<tr class='dataRow'"
                                  . " data-phone='"   . htmlspecialchars($m['mobileNumber']) . "'"
                                  . " data-nin='"     . htmlspecialchars($m['nin'])          . "'"
                                  . " data-fname='"   . htmlspecialchars($m['fname'])        . "'"
                                  . " data-mname='"   . htmlspecialchars($m['mname'])        . "'"
                                  . " data-lname='"   . htmlspecialchars($m['lname'])        . "'"
                                  . " data-day='"     . htmlspecialchars($m['day'])          . "'"
                                  . " data-month='"   . htmlspecialchars($m['month'])        . "'"
    ...
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        ...
        const statusCounts = [
          <?php echo (int)$activeLoans; ?>,
          <?php echo (int)$pendingLoans; ?>,
          <?php echo (int)$paidLoans; ?>,
          <?php echo (int)$overdueLoans; ?>
        ];

        const masterStatusChart = new ApexCharts(
          document.querySelector('#loanStatusDoughnutMaster'), {
          chart: { height: 350, type: 'donut', parentHeightOffset: 0 },
          series: statusCounts,
          labels: statusLabels,
          ...
          dataLabels: {
            enabled: true,
            style: { fontSize: '12px', fontFamily: 'Inter', colors: ['#fff'] },
            ...
            pie: {
              donut: {
                ...
                labels: {
                  show: true,
                  name: {
                    show: true, fontSize: '14px', fontFamily: 'Inter',
                    color: '#6c757d', offsetY: -8
                  },
                  value: {
                    show: true, fontSize: '24px', fontFamily: 'Inter',
                    fontWeight: '600', color: '#495057', offsetY: 8,
                    formatter: function (val) { return parseInt(val).toLocaleString(); }
                  },
                  total: {
                    show: true, label: 'Total Record Count',
                    fontSize: '13px', fontFamily: 'Inter', color: '#878a99',
          ...
          responsive: [{ breakpoint: 600, options: { chart: { height: 280 }, legend: { position: 'bottom' } } }]
        });
        masterStatusChart.render();

        // ── Line – Share Deposits ───────────────────────────────────────────
        const shareTrendChart = new ApexCharts(
          document.querySelector('#shareTrendLine'), {
        ...
        // ── Line – Loan Issuance (total_payable) ────────────────────────────
        const loanTrendChart = new ApexCharts(
          document.querySelector('#loanTrendLine'), {
          chart: { height: 160, type: 'line', toolbar: { show: false } },
          series: [{ name: 'Total Payable', data: <?php echo json_encode($amount); ?> }],
          stroke: { curve: 'smooth', width: 2 },
          markers: { size: 4 },
    ...
    <style>
      ...
      #ai-assistant-btn:hover  { transform: translateY(-3px) scale(1.07); box-shadow: 0 8px 28px rgba(0,36,107,0.6); }
      #ai-assistant-btn:active { transform: scale(0.96); }
      #ai-assistant-btn svg    { width: 26px; height: 26px; fill: #ffffff; }
    ...
    <script>
      function toggleAiChat() {
        document.getElementById('ai-chat-modal').classList.toggle('open');
      }
